...
class HICODet(ImageDataset):
    ...
    def __getitem__(self, i: int) -> tuple:
        ...
        #intra_idx = self.start
        #print(self._filenames[intra_idx])
        #self.start+=1

        anno = self._anno[intra_idx].copy()
        anno.pop("hoi")
        objlist = []
        for objid in list(anno["object"]):
            objlist.append(self.label2id[self._hico_object[objid].replace("_", " ")])
        anno["object"] = torch.tensor(objlist)
        #print(anno)
        #self.hoi_annotaion[(obj, verb)]
        aff_list = []
        hoi_list = []
        for objid, verbid in zip(anno["object"], anno["verb"]):
            #print(objid.item())
            #print(str(objid.item()))
            objstr = self.id2label[str(objid.item())]
            verbstr = self._verbs[verbid]
            aff_list.append(self.hoi_annotation[(objstr.replace(" ", "_"), verbstr)])
            hoi_list.append(objid * 3 + aff_list[-1])

        anno["verb"] = torch.tensor(aff_list)
        anno["hoi"] = hoi_list
        ...
        for hbox, obox, objidx, verbidx in zip(anno["boxes_h"], anno["boxes_o"], anno["object"], anno["verb"]):
            found = False
            for merged_idx, (merged_hbox, merged_obox, merged_o, merged_v) in enumerate(zip(merged_boxes_h, merged_boxes_o, merged_obj, merged_verb)):
                hbox_overlap = get_iou({"x1": hbox[0], "x2": hbox[2], "y1": hbox[1], "y2": hbox[3]}, {"x1": merged_hbox[0], "x2": merged_hbox[2], "y1": merged_hbox[1], "y2": merged_hbox[3]})
                obox_overlap = get_iou({"x1": obox[0], "x2": obox[2], "y1": obox[1], "y2": obox[3]}, {"x1": merged_obox[0], "x2": merged_obox[2], "y1": merged_obox[1], "y2": merged_obox[3]})

                if hbox_overlap > 0.5 and obox_overlap > 0.5 and objidx == merged_o:
                    if merged_v < verbidx:
                        merged_verb[merged_idx] = verbidx
                    found = True
                    break
            if not found:
                merged_boxes_h.append(hbox)
                merged_boxes_o.append(obox)
                merged_obj.append(objidx)
                merged_verb.append(verbidx)

        anno["boxes_h"] = torch.tensor(merged_boxes_h)
        anno["boxes_o"] = torch.tensor(merged_boxes_o)
        anno["object"] = torch.tensor(merged_obj)
        anno["verb"] = torch.tensor(merged_verb)
...
[90]
    ...
    def _load_annotation_and_metadata(self, f: dict) -> None:
        ...
        for anno in self._anno:
            for objid, verbid in zip(anno["object"], anno["verb"]):
                objstr = self._hico_object[objid]
